...
function openLeadForm(initialPhone="", existing=null) {
  ...
  conditionInput.addEventListener("input", () => {
    ...
    if (conditionInput.value.includes("방문")) dlg.querySelector('[name="visitStatus"]').checked=true;
  });
  updateExistingCustomer();
...
document.addEventListener("keydown", event => {
  ...
  if (event.code==="KeyS" && event.ctrlKey && leadDialog) {
    event.preventDefault();
    const saveButton=leadDialog.querySelector("#saveLead");
    if (saveButton && !saveButton.disabled) saveButton.click();
    return;
  }
  if (event.code==="KeyN" && !event.ctrlKey && !event.metaKey && !event.altKey && !event.shiftKey && !leadDialog && !isTextEntryTarget(event.target) && isLoggedIn()) {
    event.preventDefault();
    openLeadForm();
  }
});
