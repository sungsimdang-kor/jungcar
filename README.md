...
- 일반 상담 저장은 JSONP 1회로 처리하고, URL 제한을 넘는 긴 메모만 POST를 사용합니다.
- 응답이 유실되면 `verify`로 해당 행을 읽어 확인합니다. 즉시 두 번째 쓰기 요청을 보내지 않습니다.
- 서버가 반환한 항목 불일치·인증 오류는 숨기지 않고 표시하며, 이 경우 즉시 자동 재시도하지 않습니다.
